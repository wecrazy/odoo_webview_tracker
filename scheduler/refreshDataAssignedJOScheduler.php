...
class refreshDataAssignedJOScheduler
{
    private $log;
    private $config;
    private $error;
    private $mailer;
    private $assignedJOInstance;
    ...
    public function __construct()
    {
       ...
       -----------------------*/
        ...
        $paths = [
            __DIR__ . '/controllers/web/assigned_jo/assignedJOController.php',
            dirname(__DIR__) . '/controllers/web/assigned_jo/assignedJOController.php',
            dirname(__DIR__, 2) . '/controllers/web/assigned_jo/assignedJOController.php',
        ];
        ...
        if (!$pathFound) {
            throw new \Exception('Assigned JO Controller path not found!');
        }

        $this->assignedJOInstance = new assignedJOController();
    }

    public function refreshData() {
        $response = $this->assignedJOInstance->getNewAssignedJO();
        if ($response) {
            $this->log->createLogMessage("--- SUCCESS : Assigned JO Success Updated!");
        }
    }
}

$techniciansInstance = new refreshDataAssignedJOScheduler();
